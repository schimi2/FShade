...
module Optimization =
    ...
    let rec eliminateDeadCode (e : Expr) =
        ...
        while !r do
            ...
            let state = { compiler = Unchecked.defaultof<ICompiler<EliminationState>>; types = PersistentHashSet.empty; functions = PersistentHashSet.empty; functionId = 0; constantId = 0; constants = HashMap.empty; lambdaId = 0; lambdas = Map.empty; uniformId = 0; uniforms = HashMap.empty; defines = Map.empty; bound = Set.empty; userState = { used = free; found = false }; initialUserState = { used = free; found = false }}
